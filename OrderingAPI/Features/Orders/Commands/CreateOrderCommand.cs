...
public class CreateOrderCommandHandler(AppDbContext dbContext) : IRequestHandler<CreateOrderCommand, OrderModel>
{
    public async ValueTask<OrderModel> Handle(CreateOrderCommand command, CancellationToken cancellationToken)
    {
        ...
        var newOrder = Order.Create(command.CustomerId, command.Lines.Select(line => ToOrderLine(line, normalizedSkuMap)).ToList(),
    ...
    private async Task<Dictionary<string, Domain.Entities.ProductCache>> ResolveProductCacheMapAsync(
        IEnumerable<OrderLineModel> lines,
    ...
    private static OrderLine ToOrderLine(OrderLineModel model, IReadOnlyDictionary<string, Domain.Entities.ProductCache> productsBySku)
    {
        var sku = new Sku(model.Sku);
        var orderLine = new OrderLine(
            sku,
            Quantity.Of(model.Quantity),
            ResolveUnitPrice(model, productsBySku, sku.Value));
    ...
    private static Money ResolveUnitPrice(OrderLineModel model,
        IReadOnlyDictionary<string, Domain.Entities.ProductCache> productsBySku,
    ...
    private static OrderModel ToOrderModel(Order order, IReadOnlyDictionary<string, Domain.Entities.ProductCache> productsBySku)
    {
