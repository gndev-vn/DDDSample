...
public class GetProductsQueryHandler(AppDbContext dbContext) : IRequestHandler<GetProductsQuery, List<ProductResponse>>
{
    public async ValueTask<List<ProductResponse>> Handle(GetProductsQuery query, CancellationToken cancellationToken)
    {
        ...
        return await productsQuery
            ...
            {
                Id = product.Id,
                CategoryId = EF.Property<Guid?>(product, "CategoryId") ?? Guid.Empty,
                Name = product.Name,
                Slug = product.Slug,
                Description = product.Description,
                BasePrice = product.BasePrice != null ? product.BasePrice.Amount : 0,
                Currency = product.BasePrice != null ? product.BasePrice.Currency : string.Empty,
