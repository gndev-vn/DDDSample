...
namespace CatalogAPI.Features.Category.Commands;

public record UpdateCategoryCommand(CategoryModel Category) : IRequest<CategoryModel>;
    ...
    : IRequestHandler<UpdateCategoryCommand, CategoryModel>
{
    ...
    {
        var category = await dbContext.Categories.FindAsync([command.Category.Id], cancellationToken: cancellationToken);
        if (category == null)
        {
            throw new KeyNotFoundException();
        }

        command.Category.Adapt(category);
